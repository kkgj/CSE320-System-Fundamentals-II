...
Test(sfmm_student_suite, student_test_1, .timeout = TEST_TIMEOUT) {
	void *a = sf_malloc(sizeof(long double));
...
Test(sfmm_student_suite, student_test_2, .timeout = TEST_TIMEOUT) {
	void *b = sf_malloc(sizeof(int) * 7);
	sf_malloc(sizeof(double));

	cr_assert_not_null(b, "b is NULL!");

	sf_block *bp = (sf_block *)((char*)b - 2*sizeof(sf_header));
	cr_assert((bp->header ^ MAGIC) & THIS_BLOCK_ALLOCATED, "Allocated bit is not set!");
	assert_quick_list_block_count(0, 0);
...
Test(sfmm_student_suite, student_test_4, .timeout = TEST_TIMEOUT) {
    double* ptr1 = sf_malloc(sizeof(short)*2000);
    sf_free(ptr1);
...
Test(sfmm_student_suite, student_test_5, .timeout = TEST_TIMEOUT) {
    double* ptr1 = sf_malloc(sizeof(unsigned long)*25);
    sf_realloc(ptr1, 300);
