...
Test(basecode_tests_suite, goertzel_additional_test) {
    FILE *fp = fopen("./rsrc/dtmf_0_500ms.au","r");
    AUDIO_HEADER audio_test; //= (AUDIO_HEADER*)malloc(24);
    audio_read_header(fp, &audio_test);
    int N = 1000;
    double k = 697.0 * N / 8000;
    GOERTZEL_STATE g0;
    goertzel_init(&g0, N, k);
    int16_t sample;
    for(int i = 0; i < N-1; i++) {
        audio_read_sample(fp, &sample);
        goertzel_step(&g0, (double)sample / INT16_MAX);
    }
    audio_read_sample(fp, &sample);
    double r0 = goertzel_strength(&g0, (double)sample / INT16_MAX);
    printf("result is %f\n", r0);
    fclose(fp);
    double eps = 1e-6;
    cr_assert((fabs(r0-0.000003) < eps), "r1 was %f, should be 0.5", r0);
}
